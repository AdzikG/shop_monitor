...
class Cart3Rules(BaseRules):
    def check(self, run_data: RunData) -> RulesResult:
        ...
        if self.scenario_context.postal_code and cart3.postal_code:
            if cart3.postal_code != self.scenario_context.postal_code:
                self.add_alert(
                    'CART3_POSTAL_MISMATCH',
                    f'Kod pocztowy: oczekiwano {self.scenario_context.postal_code}, jest {cart3.postal_code}',
                )
